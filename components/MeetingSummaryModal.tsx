      ...
      >
        ...
        <div className={`border-b border-gray-700 p-6 ${
          ...
          <div className="text-center">
            ...
            <p className="text-lg text-gray-300">{meetingTitle || 'Meeting Summary'}</p>
            {earlyDeparture && (
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-250px)] p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Participation Score */}
            <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-5 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                <TrendingUp className={`w-5 h-5 ${getScoreColor(participationScore)}`} />
                <span className="text-sm text-gray-400 uppercase tracking-wide">Participation Score</span>
              </div>
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: animateScore ? 1 : 0 }}
                transition={{ type: 'spring', stiffness: 200, damping: 15, delay: 0.3 }}
                className={`text-5xl font-bold ${getScoreColor(participationScore)} mb-1`}
              >
                {participationScore}
              </motion.div>
              <p className="text-xs text-gray-400">{getScoreMessage(participationScore)}</p>
            </div>
            ...
            <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-5 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-400 uppercase tracking-wide">XP Earned</span>
              </div>
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: animateScore ? 1 : 0 }}
                transition={{ type: 'spring', stiffness: 200, damping: 15, delay: 0.5 }}
                className={`text-5xl font-bold mb-1 ${earlyDeparture ? 'text-yellow-400' : 'text-green-400'}`}
              >
                +{xpEarned}
              </motion.div>
              ...
              {feedback.strengths && feedback.strengths.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-sm font-semibold text-green-400 uppercase tracking-wide">Strengths</h4>
                  </div>
              ...
              {feedback.improvements && feedback.improvements.length > 0 && (
                <div className="pt-3 border-t border-gray-700">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-sm font-semibold text-yellow-400 uppercase tracking-wide">Areas for Improvement</h4>
                  </div>
                  <ul className="space-y-2">
          ...
          {earlyDeparture && (
            <div className="bg-yellow-500/10 border border-yellow-500/50 rounded-lg p-5">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
                <div>
                  <h3 className="text-lg font-bold text-white mb-2">Early Departure</h3>
          ...
          {!earlyDeparture && generatedTasks && generatedTasks.length > 0 && (
            <div className="bg-indigo-500/10 border border-indigo-500/50 rounded-lg p-5">
              <div className="flex items-center gap-2 mb-4">
                <ListTodo className="w-5 h-5 text-indigo-400" />
                <h3 className="text-lg font-bold text-white">New Tasks Generated</h3>
              </div>
                ...
                {generatedTasks.map((task, index) => {
                  ...
                  return (
                    <motion.li
                      ...
                      className="bg-gray-900/50 border border-gray-700 rounded-lg p-3 flex items-center justify-between hover:border-indigo-500/50 transition-colors"
                    >
